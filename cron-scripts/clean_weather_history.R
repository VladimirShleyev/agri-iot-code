...
# почему-то история начинает пропадать, пока отключили перезапись
write_lines(w_raw_data$txt, 
           paste0(tools::file_path_sans_ext(ifname), "_mod.txt"), #кидаем поверх существующего файла (было _mod.txt)
           append = FALSE)
